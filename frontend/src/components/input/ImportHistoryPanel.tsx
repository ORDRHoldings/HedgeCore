    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
      ...
      {runs.map(run => (
        <div key={run.id}>
          ...
          >
            <span style={{ fontFamily: S.fontMono, fontSize: "0.5625rem", color: S.primary, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {run.source_filename ?? "—"}
            </span>
            <span style={{
            ...
            <span style={{ fontFamily: S.fontMono, fontSize: "0.5625rem", color: S.secondary }}>
              {run.total_rows}
            </span>
            <span style={{ fontFamily: S.fontMono, fontSize: "0.5625rem", color: S.green }}>
              {run.created_ok}
            </span>
            <span style={{ fontFamily: S.fontMono, fontSize: "0.5625rem", color: run.error_count > 0 ? S.red : S.tertiary }}>
              {run.error_count}
            </span>
            <span style={{ fontFamily: S.fontMono, fontSize: "0.5rem", color: statusColor(run.status) }}>
          ...
          {expanded === run.id && (
            <div style={{
              ...
              <div style={{ display: "flex", gap: 24, flexWrap: "wrap", marginBottom: 10 }}>
                {[
                  ...
                  <div key={label} style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                    <span style={{ fontFamily: S.fontMono, fontSize: "0.375rem", letterSpacing: "0.08em", color: S.tertiary }}>{label}</span>
                    <span style={{ fontFamily: S.fontMono, fontSize: "0.5rem", color: S.secondary }}>{val}</span>
                  </div>
                ))}
              </div>
              ...
              {run.error_count > 0 && (
                <>
                  <div style={{ fontFamily: S.fontMono, fontSize: "0.4375rem", letterSpacing: "0.08em", color: S.tertiary, marginBottom: 6 }}>
                    ROW ERRORS
                  </div>
                  {detailLoading && (
                    <div style={{ fontFamily: S.fontMono, fontSize: "0.5rem", color: S.tertiary }}>Loading errors…</div>
                  )}
              ...
              {run.error_count === 0 && (
                <div style={{ fontFamily: S.fontMono, fontSize: "0.5rem", color: S.green }}>

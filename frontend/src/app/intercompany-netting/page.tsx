...
// Local palette for shades not in T: button-on-accent white, status-pill
// signal hues, dark-red error toast. Status colors are tuned to remain
// recognisable as netting workflow states (amber pending, green ok, etc.).
const C = {
  white:      "#fff",
  blueAccent: "#2563eb",
  blue:       "#3b82f6",
...
function NettingInner() {
  ...
  return (
    <div style={{ fontFamily: S.fontUI, padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      ...
      {error && <div style={{ background: C.toastBg, color: C.toastFg, padding: "8px 14px", borderRadius: 6, marginBottom: 16, fontSize: 13 }}>{error}<button onClick={() => setError(null)} style={{ float: "right", background: "none", border: "none", color: C.toastFg, cursor: "pointer" }}>x</button></div>}
        ...
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 16px", fontFamily: S.fontMono, fontSize: 12, fontWeight: 600, letterSpacing: 0.5, textTransform: "uppercase",
              background: tab === t.key ? S.bgPanel : "transparent", color: tab === t.key ? C.white : T.secondary,
      ...
      {loading && <div style={{ color: T.secondary, fontSize: 13, padding: 20 }}>Loading...</div>}

      {/* OBLIGATIONS TAB */}
      {tab === "OBLIGATIONS" && !loading && (
        <div>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <button onClick={() => setShowForm(!showForm)} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: C.blueAccent, color: C.white, border: "none", borderRadius: 6, fontSize: 13, fontFamily: S.fontMono, cursor: "pointer" }}>
          ...
          {showForm && (
            <div style={{ background: S.bgSub, border: `1px solid ${S.rim}`, borderRadius: 8, padding: 16, marginBottom: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
                <label style={{ fontSize: 12, fontFamily: S.fontMono }}>
                  ...
                  <select value={form.debtor_entity_id} onChange={e => setForm({ ...form, debtor_entity_id: e.target.value })}
                    style={{ width: "100%", padding: 8, marginTop: 4, background: S.bgDeep, color: C.white, border: `1px solid ${S.rim}`, borderRadius: 4, fontSize: 13 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button onClick={handleCreate} style={{ padding: "8px 16px", background: C.blueAccent, color: C.white, border: "none", borderRadius: 6, fontSize: 13, fontFamily: S.fontMono, cursor: "pointer" }}>CREATE</button>
                <button onClick={() => setShowForm(false)} style={{ padding: "8px 16px", background: "transparent", color: T.secondary, border: `1px solid ${S.rim}`, borderRadius: 6, fontSize: 13, fontFamily: S.fontMono, cursor: "pointer" }}>CANCEL</button>
              </div>
            </div>
          )}
              ...
              <thead>
                <tr style={{ borderBottom: `1px solid ${S.rim}` }}>
                  {["DEBTOR", "CREDITOR", "AMOUNT", "CCY", "DUE DATE", "REF", "STATUS", ""].map(h => (
                    <th scope="col" key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 600, color: T.secondary, letterSpacing: 0.5 }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {obligations.length === 0 && (
                  <tr><td colSpan={8} style={{ padding: 20, textAlign: "center", color: T.tertiary }}>No obligations found</td></tr>
                )}
                {obligations.map(o => (
                  <tr key={o.id} style={{ borderBottom: `1px solid ${S.rim}22` }}>
                    ...
                    <td style={{ padding: "8px 12px", color: T.secondary }}>{o.reference || "\u2014"}</td>
                    <td style={{ padding: "8px 12px" }}>
                      <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600, background: `${statusColor[o.status] || T.tertiary}22`, color: statusColor[o.status] || T.tertiary }}>{o.status}</span>
                    </td>
                    <td style={{ padding: "8px 12px" }}>
                      {o.status === "PENDING" && (
                        <button onClick={() => handleCancel(o.id)} title="Cancel" style={{ background: "none", border: "none", color: C.red, cursor: "pointer", padding: 4 }}><X size={14} /></button>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {tab === "PROPOSALS" && !loading && (
        <div>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 12 }}>
            <button onClick={handleGenerate} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 14px", background: C.blueAccent, color: C.white, border: "none", borderRadius: 6, fontSize: 13, fontFamily: S.fontMono, cursor: "pointer" }}>
          ...
          <div style={{ background: S.bgPanel, border: `1px solid ${S.rim}`, borderRadius: 8, overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: S.fontMono }}>
              ...
              <tbody>
                {proposals.length === 0 && (
                  <tr><td colSpan={9} style={{ padding: 20, textAlign: "center", color: T.tertiary }}>No proposals -- generate from pending obligations</td></tr>
                )}
                {proposals.map(p => (
                  <tr key={p.id} style={{ borderBottom: `1px solid ${S.rim}22` }}>
                    ...
                    <td style={{ padding: "8px 12px", color: C.green, fontWeight: 600, textAlign: "right" }}>{fmtAmount(p.savings)}</td>
                    <td style={{ padding: "8px 12px" }}>
                      <span style={{ padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600, background: `${statusColor[p.status] || T.tertiary}22`, color: statusColor[p.status] || T.tertiary }}>{p.status}</span>
                    </td>
                    <td style={{ padding: "8px 12px", display: "flex", gap: 4 }}>
                      {p.status === "PENDING_APPROVAL" && (
                        <button onClick={() => handleApprove(p.id)} title="Approve" style={{ background: "none", border: "none", color: C.green, cursor: "pointer", padding: 4 }}><Check size={14} /></button>
                      )}
                      {p.status === "APPROVED" && (
                        <button onClick={() => handleExecute(p.id)} title="Execute" style={{ background: "none", border: "none", color: C.blue, cursor: "pointer", padding: 4 }}><Play size={14} /></button>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {tab === "SAVINGS" && !loading && (
        <div>
          {savings ? (
            <div>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 16, marginBottom: 24 }}>
                <div style={{ background: S.bgPanel, border: `1px solid ${S.rim}`, borderRadius: 8, padding: 20 }}>
                  <div style={{ fontSize: 11, fontFamily: S.fontMono, color: T.secondary, fontWeight: 600, letterSpacing: 0.5, marginBottom: 8 }}>TOTAL SAVINGS</div>
                  <div style={{ fontSize: 28, fontFamily: S.fontMono, fontWeight: 700, color: C.green }}>{fmtAmount(savings.total_savings)}</div>
                </div>
                <div style={{ background: S.bgPanel, border: `1px solid ${S.rim}`, borderRadius: 8, padding: 20 }}>
                  <div style={{ fontSize: 11, fontFamily: S.fontMono, color: T.secondary, fontWeight: 600, letterSpacing: 0.5, marginBottom: 8 }}>NETTINGS EXECUTED</div>
                  <div style={{ fontSize: 28, fontFamily: S.fontMono, fontWeight: 700 }}>{savings.netting_count}</div>
                </div>
                <div style={{ background: S.bgPanel, border: `1px solid ${S.rim}`, borderRadius: 8, padding: 20 }}>
                  <div style={{ fontSize: 11, fontFamily: S.fontMono, color: T.secondary, fontWeight: 600, letterSpacing: 0.5, marginBottom: 8 }}>AVG SAVINGS / NETTING</div>
                  <div style={{ fontSize: 28, fontFamily: S.fontMono, fontWeight: 700, color: C.green }}>
              ...
              {Object.keys(savings.savings_by_currency).length > 0 && (
                <div style={{ background: S.bgPanel, border: `1px solid ${S.rim}`, borderRadius: 8, overflow: "hidden" }}>
                  <div style={{ padding: "12px 16px", borderBottom: `1px solid ${S.rim}`, fontFamily: S.fontMono, fontSize: 12, fontWeight: 600, letterSpacing: 0.5, color: T.secondary }}>SAVINGS BY CURRENCY</div>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13, fontFamily: S.fontMono }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${S.rim}` }}>
                        <th scope="col" style={{ padding: "10px 16px", textAlign: "left", fontSize: 11, fontWeight: 600, color: T.secondary }}>CURRENCY</th>
                        <th scope="col" style={{ padding: "10px 16px", textAlign: "right", fontSize: 11, fontWeight: 600, color: T.secondary }}>SAVINGS</th>
                      </tr>
                    </thead>
                    <tbody>
                      {Object.entries(savings.savings_by_currency).map(([ccy, amt]) => (
                        <tr key={ccy} style={{ borderBottom: `1px solid ${S.rim}22` }}>
                          ...
                          <td style={{ padding: "8px 16px", textAlign: "right", color: C.green, fontWeight: 600 }}>{fmtAmount(amt)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div style={{ color: T.tertiary, textAlign: "center", padding: 40 }}>No savings data yet -- execute netting proposals to track savings</div>
          )}
        </div>
      )}
    </div>
...
export default function IntercompanyNettingPage() {
  return (
    <Suspense fallback={<div style={{ padding: 40, color: T.tertiary }}>Loading...</div>}>
      <NettingInner />
    </Suspense>
  );
}

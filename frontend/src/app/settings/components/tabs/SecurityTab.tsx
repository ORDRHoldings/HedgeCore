...
export default function SecurityTab({ token }: Props) {
  ...
  const errBox = (msg: string) => (
    <div style={{ background: `color-mix(in srgb, ${S.fail} 8%, transparent)`, border: `1px solid ${S.fail}`, borderLeft: `3px solid ${S.fail}`, borderRadius: 2, padding: "8px 12px", fontFamily: S.fontMono, fontSize: 12, color: S.fail, letterSpacing: "0.06em" }}>
      ✗ {msg}
    </div>
  );

  if (loadingStatus) return <div style={{ padding: "40px 0", textAlign: "center", fontFamily: S.fontMono, fontSize: 12, color: S.tertiary, letterSpacing: "0.09em" }}>LOADING MFA STATUS…</div>;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
        <span style={{ fontFamily: S.fontMono, fontSize: 12, color: S.secondary, letterSpacing: "0.06em" }}>STATUS:</span>
        {mfaStatus?.is_enabled ? (
          <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, letterSpacing: "0.09em", color: S.pass, background: `color-mix(in srgb, ${S.pass} 10%, transparent)`, border: `1px solid color-mix(in srgb, ${S.pass} 30%, transparent)`, borderRadius: 2, padding: "2px 8px" }}>● ENABLED</span>
        ) : (
          <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, letterSpacing: "0.09em", color: S.tertiary, background: S.bgSub, border: `1px solid ${S.rim}`, borderRadius: 2, padding: "2px 8px" }}>○ NOT ENABLED</span>
        )}
      </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {!showDisable ? (
            <div>
              ...
              <div style={{ fontFamily: S.fontUI, fontSize: 12, color: S.tertiary, marginTop: 6 }}>Disabling MFA reduces account security. You will be required to enter your authenticator code to confirm.</div>
            </div>
          ) : (
            <div style={{ background: `color-mix(in srgb, ${S.fail} 5%, transparent)`, border: `1px solid color-mix(in srgb, ${S.fail} 20%, transparent)`, borderLeft: `3px solid ${S.fail}`, borderRadius: 2, padding: "16px 18px", display: "flex", flexDirection: "column", gap: 12 }}>
              <div style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.fail, letterSpacing: "0.09em" }}>CONFIRM MFA DISABLE</div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <button onClick={() => { setShowDisable(false); setDisableCode(""); setDisableError(null); }} style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", color: S.secondary, background: "transparent", border: `1px solid ${S.rim}`, borderRadius: 2, padding: "8px 14px", cursor: "pointer" }}>CANCEL</button>
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {!setupData ? (
            ...
            <div style={{ background: `color-mix(in srgb, ${S.cyan} 4%, transparent)`, border: `1px solid color-mix(in srgb, ${S.cyan} 15%, transparent)`, borderLeft: `3px solid ${S.cyan}`, borderRadius: 2, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.cyan, letterSpacing: "0.09em" }}>ENROLL AUTHENTICATOR</div>
              ...
              {setupData.provisioning_uri && (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.tertiary, letterSpacing: "0.09em" }}>SCAN QR CODE:</span>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.tertiary, letterSpacing: "0.09em" }}>TOTP SECRET:</span>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button onClick={() => handleCopy(setupData.secret)} style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, letterSpacing: "0.06em", color: copied ? S.pass : S.secondary, background: S.bgSub, border: `1px solid ${copied ? S.pass : S.rim}`, borderRadius: 2, padding: "8px 12px", cursor: "pointer", flexShrink: 0 }}>{copied ? "COPIED ✓" : "COPY"}</button>
                </div>
              </div>
              {setupData.backup_codes.length > 0 && (
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.amber, letterSpacing: "0.09em" }}>BACKUP CODES — STORE SECURELY:</span>
                  <div style={{ background: S.bgDeep, border: `1px solid ${S.rim}`, borderRadius: 2, padding: "10px 14px", display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {setupData.backup_codes.map((code, i) => (
                      <span key={i} style={{ fontFamily: S.fontMono, fontSize: 12, color: S.amber, background: `color-mix(in srgb, ${S.amber} 8%, transparent)`, border: `1px solid color-mix(in srgb, ${S.amber} 20%, transparent)`, borderRadius: 2, padding: "2px 8px", letterSpacing: "0.12em" }}>{code}</span>
                    ))}
                  </div>
                </div>
              )}
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.tertiary, letterSpacing: "0.09em" }}>ENTER 6-DIGIT CODE TO ACTIVATE:</span>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <div style={{ background: S.bgSub, border: `1px solid ${S.soft}`, borderRadius: 2, padding: "10px 14px", fontFamily: S.fontUI, fontSize: 12, color: S.tertiary, lineHeight: 1.6 }}>
        <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, color: S.secondary, marginRight: 6, letterSpacing: "0.07em" }}>TOTP</span>
      ...
      <div style={{ borderTop: `1px solid ${S.soft}`, paddingTop: 24, display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <span style={{ fontFamily: S.fontMono, fontSize: 12, fontWeight: 700, letterSpacing: "0.07em", color: ipEnabled ? S.cyan : S.tertiary }}>
            EXECUTION IP ALLOWLIST {ipEnabled ? "ENABLED" : "DISABLED"}
          </span>
        </div>

        <div style={{ fontFamily: S.fontUI, fontSize: 12, color: S.tertiary, lineHeight: 1.6 }}>

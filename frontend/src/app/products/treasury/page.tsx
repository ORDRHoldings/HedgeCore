...
import {
  LayoutGrid, Cpu, Users, Shield, FileCheck, Settings, FileSpreadsheet,
...
const FEATURES = [
  { icon: <Cpu size={22} />, title: "Deterministic Engine", desc: "Same inputs = same outputs, always. 41 production modules, sub-50ms computation. Auditor-friendly, explainable, reproducible." },
  { icon: <Users size={22} />, title: "4-Eyes Governance", desc: "Maker-checker approval with Separation of Duties. Sandbox to Staging to Ledger pipeline. No single point of failure." },
  { icon: <Shield size={22} />, title: "WORM Audit Trail", desc: "Append-only event log with SHA-256 hash chain per tenant. Tamper-evident, regulation-proof audit semantics." },
  { icon: <FileCheck size={22} />, title: "IFRS 9 / ASC 815", desc: "Built-in prospective effectiveness testing, hedge documentation, critical terms matching, accounting framework alignment." },
  { icon: <Settings size={22} />, title: "Policy Engine", desc: "60 policy presets with maturity profiles, governance tiers, evidence grades. Extended overlays: volatility, geopolitical, netting." },
  { icon: <FileSpreadsheet size={22} />, title: "Report Studio", desc: "30+ report templates, split-pane studio, PDF/XLSX export, regulatory format downloads (EMIR, MiFID, Dodd-Frank)." },
];

const METRICS = [
  { value: "41", label: "Engine Modules" },
  { value: "60", label: "Policy Presets" },
  { value: "30", label: "Report Templates", suffix: "+" },
  { value: "50", label: "ms Computation", prefix: "<" },
];

const USE_CASES = [
  { title: "For Treasury Teams", desc: "End-to-end hedge lifecycle management from exposure import through governed execution. Replace spreadsheets with institutional infrastructure." },
  { title: "For Risk Officers", desc: "R1-R8 risk classification, scenario stress testing, and real-time portfolio decomposition. Full audit trail for regulatory reporting." },
  { title: "For Auditors", desc: "Deterministic, reproducible calculations with SHA-256 hash-chained audit events. WORM semantics guarantee tamper-evident records." },
];
  ...
  return (
    <MarketingLayout>
      <style>{`
        ...
        .feat-card:hover{transform:translateY(-4px);border-color:${dk ? "rgba(34,211,238,0.25)" : "rgba(30,58,95,0.2)"} !important;box-shadow:${dk ? "0 8px 40px rgba(34,211,238,0.08)" : "0 8px 40px rgba(30,58,95,0.08)"}}
      ...
      <section style={{ padding: mob ? "32px 20px 48px" : "56px 48px 72px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        {dk && <div style={{ position: "absolute", top: "5%", left: "25%", width: 500, height: 500, borderRadius: "50%", background: `radial-gradient(circle, ${COLOR}08, transparent 60%)`, pointerEvents: "none", animation: "o-float 12s ease infinite" }} />}
        <div style={{ position: "relative", zIndex: 1, maxWidth: 800, margin: "0 auto", opacity: heroVis ? 1 : 0, transform: heroVis ? "translateY(0)" : "translateY(24px)", transition: "all .7s ease" }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 36, flexWrap: "wrap" }}>
            <Link href="/auth/login" style={{ fontFamily: F.ui, fontSize: 15, fontWeight: 600, color: T.accentText, background: T.accent, padding: "13px 32px", borderRadius: 10, textDecoration: "none", display: "flex", alignItems: "center", gap: 8, boxShadow: dk ? `0 0 30px ${COLOR}25` : "0 4px 16px rgba(30,58,95,0.15)" }}>
              Start Managing Your Hedge Portfolio <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ marginBottom: mob ? 36 : 56, opacity: featuresView.visible ? 1 : 0, transform: featuresView.visible ? "translateY(0)" : "translateY(20px)", transition: "all .6s ease" }}>
            ...
            <h2 style={{ fontFamily: F.heading, fontSize: mob ? 32 : 44, fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, margin: 0, color: T.text }}>
              Institutional infrastructure, not a spreadsheet
            </h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "repeat(3, 1fr)", gap: mob ? 12 : 16 }}>
            {FEATURES.map((feat, i) => (
              <div key={feat.title} className="feat-card" style={{
                background: T.bgCard, border: `1px solid ${T.borderSoft}`, borderRadius: 14,
                padding: mob ? "24px 20px" : "28px 24px",
                opacity: featuresView.visible ? 1 : 0, transform: featuresView.visible ? "translateY(0)" : "translateY(20px)",
                transition: `all .5s ease ${i * 0.08}s`,
              }}>
                <div style={{ width: 42, height: 42, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", background: `${COLOR}${dk ? "12" : "0a"}`, color: COLOR, border: `1px solid ${COLOR}${dk ? "20" : "15"}`, marginBottom: 16 }}>
                  {feat.icon}
                </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "repeat(3, 1fr)", gap: mob ? 12 : 16 }}>
            {USE_CASES.map((uc, i) => (
              <div key={uc.title} style={{
                background: T.bgCard, border: `1px solid ${T.borderSoft}`, borderRadius: 14,
                padding: mob ? "24px 20px" : "28px 24px",
                opacity: useCaseView.visible ? 1 : 0, transform: useCaseView.visible ? "translateY(0)" : "translateY(20px)",
      ...
      <section ref={ctaView.ref} style={{ background: dk ? T.sectionAlt : T.ctaBg, padding: mob ? "64px 20px" : "96px 48px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 600, margin: "0 auto", opacity: ctaView.visible ? 1 : 0, transform: ctaView.visible ? "translateY(0)" : "translateY(20px)", transition: "all .6s ease" }}>
          <h2 style={{ fontFamily: F.heading, fontSize: mob ? 32 : 48, fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, margin: 0, ...(dk ? { background: `linear-gradient(135deg, #f0f0f5, ${COLOR}, #818cf8)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" } : { color: "#fff" }) }}>
            Start managing your hedge portfolio
          </h2>
          <p style={{ fontSize: 16, color: dk ? T.textDim : "rgba(255,255,255,0.5)", marginTop: 16, lineHeight: 1.6 }}>
            Deterministic, auditable, governed. From exposure to execution.
          </p>

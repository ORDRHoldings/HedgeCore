...
import {
  PieChart, Layers, Target, Globe2, BarChart3, Zap,
...
const FEATURES = [
  { icon: <Layers size={22} />, title: "R1-R8 Risk Taxonomy", desc: "Eight risk categories: translation, transaction, economic, strategic, operational, settlement, credit, liquidity. Frozen, auditable classification." },
  { icon: <PieChart size={22} />, title: "Exposure Decomposition", desc: "Break down portfolio by currency pair, maturity, entity, and risk type. Full drill-down from aggregate to individual position." },
  { icon: <Target size={22} />, title: "Hedge Plan Generation", desc: "Automatic bucket-level hedge recommendations with coverage ratio optimization. Instrument selection and notional sizing." },
  { icon: <Globe2 size={22} />, title: "Multi-Currency Support", desc: "USD, EUR, GBP, JPY, MXN, and 100+ currency pairs. Cross-currency netting and multi-entity consolidation." },
  { icon: <BarChart3 size={22} />, title: "Concentration Analysis", desc: "Herfindahl-Hirschman Index, peak bucket identification, diversification scoring. Identify and manage concentration risk." },
  { icon: <Zap size={22} />, title: "Scenario Stress Testing", desc: "Parametric shocks across all risk factors with hedged vs unhedged comparison. Quantify downside across scenarios." },
];

const METRICS = [
  { value: "8", label: "Risk Categories" },
  { value: "100", label: "Currency Pairs", suffix: "+" },
  { value: "50", label: "ms Computation", prefix: "<" },
  { value: "0", label: "Latency Target", display: "Real-Time" },
];

const USE_CASES = [
  { title: "For Portfolio Managers", desc: "Decompose multi-currency exposure, generate hedge recommendations, and optimize coverage ratios across the entire portfolio." },
  { title: "For Risk Officers", desc: "R1-R8 classification, concentration analysis, and scenario stress testing. Full risk taxonomy with institutional-grade decomposition." },
  { title: "For CFOs", desc: "Board-ready portfolio risk reports with hedged vs unhedged P&L comparison. Clear visualization of exposure and hedge effectiveness." },
];
  ...
  return (
    <MarketingLayout>
      <style>{`
        ...
        .feat-card:hover{transform:translateY(-4px);border-color:${dk ? "rgba(129,140,248,0.25)" : "rgba(30,58,95,0.2)"} !important;box-shadow:${dk ? "0 8px 40px rgba(129,140,248,0.08)" : "0 8px 40px rgba(30,58,95,0.08)"}}
      ...
      <section style={{ padding: mob ? "32px 20px 48px" : "56px 48px 72px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        {dk && <div style={{ position: "absolute", top: "10%", left: "30%", width: 500, height: 500, borderRadius: "50%", background: `radial-gradient(circle, ${COLOR}08, transparent 60%)`, pointerEvents: "none", animation: "o-float 13s ease infinite" }} />}
        <div style={{ position: "relative", zIndex: 1, maxWidth: 800, margin: "0 auto", opacity: heroVis ? 1 : 0, transform: heroVis ? "translateY(0)" : "translateY(24px)", transition: "all .7s ease" }}>
          ...
          <p style={{ fontFamily: F.ui, fontSize: mob ? 16 : 19, color: T.textSub, maxWidth: 600, margin: "20px auto 0", lineHeight: 1.6 }}>
            Enterprise portfolio risk decomposition. R1-R8 risk taxonomy, exposure analysis, hedge plan generation, and scenario stress testing.
          </p>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 36, flexWrap: "wrap" }}>
            <Link href="/auth/login" style={{ fontFamily: F.ui, fontSize: 15, fontWeight: 600, color: T.accentText, background: T.accent, padding: "13px 32px", borderRadius: 10, textDecoration: "none", display: "flex", alignItems: "center", gap: 8, boxShadow: dk ? `0 0 30px ${COLOR}25` : "0 4px 16px rgba(30,58,95,0.15)" }}>
              Analyze Your Portfolio <ArrowRight size={16} />
            </Link>
          </div>
        </div>
      </section>
      ...
      <section ref={useCaseView.ref} style={{ background: dk ? T.sectionAlt : T.bgDeep, padding: mob ? "64px 16px" : "96px 48px" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ marginBottom: mob ? 36 : 56, opacity: useCaseView.visible ? 1 : 0, transform: useCaseView.visible ? "translateY(0)" : "translateY(20px)", transition: "all .6s ease" }}>
            ...
            <h2 style={{ fontFamily: F.heading, fontSize: mob ? 32 : 44, fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, margin: 0, color: T.text }}>Enterprise risk for every role</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: mob ? "1fr" : "repeat(3, 1fr)", gap: mob ? 12 : 16 }}>
            {USE_CASES.map((uc, i) => (
      ...
      <section ref={ctaView.ref} style={{ background: dk ? T.sectionAlt : T.ctaBg, padding: mob ? "64px 20px" : "96px 48px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 600, margin: "0 auto", opacity: ctaView.visible ? 1 : 0, transform: ctaView.visible ? "translateY(0)" : "translateY(20px)", transition: "all .6s ease" }}>
          <h2 style={{ fontFamily: F.heading, fontSize: mob ? 32 : 48, fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1.1, margin: 0, ...(dk ? { background: `linear-gradient(135deg, #f0f0f5, ${COLOR}, #22d3ee)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" } : { color: "#fff" }) }}>
            Decompose your portfolio risk
          </h2>
          <p style={{ fontSize: 16, color: dk ? T.textDim : "rgba(255,255,255,0.5)", marginTop: 16, lineHeight: 1.6 }}>R1-R8 classification, exposure analysis, and hedge plan generation.</p>

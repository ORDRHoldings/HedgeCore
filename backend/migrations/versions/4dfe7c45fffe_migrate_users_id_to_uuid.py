...
def upgrade() -> None:
    ...
    # 1️⃣ Drop existing FK constraints that depend on users.id
    ...
    # 2️⃣ Change users.id to UUID and set default generator
    op.execute("""
        ALTER TABLE users
    ...
    # 3️⃣ Convert referencing columns to UUID type
    ...
    # 4️⃣ Recreate foreign keys with UUID references
    op.execute("""

...
class ExtendedMarketSnapshot(MarketSnapshot):
    ...
    # Per-pair forward points (key = pair code, value = {bucket: points})
    pair_forward_points: dict[str, dict[str, float]] = Field(
        default_factory=dict,
        description="Forward points per currency pair. For percentage-quoted NDFs, values are % (e.g., 2.5 = +2.5%)."
    )

    # FX deltas for attribution (actual % change since last close)
    fx_deltas: dict[str, float] = Field(
        default_factory=dict,
        description="Actual FX % change per pair since last close."
    )

    # Previous close rates for delta computation
    previous_close_rates: dict[str, float] = Field(
        default_factory=dict,
        description="Previous close spot rates for FX delta computation."
    )

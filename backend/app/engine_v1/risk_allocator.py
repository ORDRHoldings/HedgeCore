...
def _build_candidates(
    ...
    market: dict[str, Any] | None = None,  # FIX-01: inject factor risk + vol surface
    ...
    # FIX-01: pre-compute factor risk contributions once (not per-bucket)

    factor_risk = _market.get("_factor_risk_contributions", {})

    vol_surface = _market.get("vol_surface", {})
    ...
    for i, action in enumerate(hedge_actions):
        ...
        # FIX-01: Delta-VaR MCTR (replaces notional x (1 - i x 0.05) linear decay)

        pair = action.get("pair", "USDMXN")

        if factor_risk and pair in factor_risk:

            # MCTR from factor model: marginal_contribution x notional

            mctr = factor_risk[pair].get("marginal_contribution", 0.0)

            risk_reduction = mctr * notional

        else:

            # VaR proxy: notional x pair_vol x sqrt(10/252) x 1.645

            # Independent of bucket index - no linear decay

            import math

            pair_vol = vol_surface.get(f"{pair}_1M", 12.5) / 100.0

            holding_period = 10  # 10-day VaR standard

            z_score = 1.645      # 95% confidence

            risk_reduction = notional * pair_vol * math.sqrt(holding_period / 252.0) * z_score
...
def allocate_hedges(
    ...
    candidates = _build_candidates(
        ...
        hedge_actions, margin_positions, liquidity_estimates, policy, market

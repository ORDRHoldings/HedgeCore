...
Rule:
    ...
    if breach > hard_threshold (2x limit):
        block STAGING
...
def check_concentration_limits(
    ...
    # NEW SAT-09: per-pair concentration limits
    pair_overrides: dict[str, float] = policy.get("pair_concentration_overrides", {})
    ...
    for instrument, notional in instrument_totals.items():
        ...
        # Use pair-specific limit if available
        effective_limit = pair_overrides.get(instrument, max_conc)
        hard_threshold_inst = effective_limit * 2.0
        excess = max(0.0, conc_pct - effective_limit)

        if conc_pct > hard_threshold_inst:
            status = "BREACH"
            has_breaches = True
            breaches.append(instrument)
        elif conc_pct > effective_limit:
            status = "WARNING"
            has_warnings = True
        else:
            status = "OK"
        ...
        checks.append(ConcentrationCheck(
            ...
            limit_pct=max_conc,          # keep showing the base limit
            status=status,
            excess_pct=excess,
        ))

...
Classification:
    ...
    STRESSED -> 2x slippage, 1.5x margin
    CRISIS  -> 5x slippage, 3x margin
...
def classify_liquidity_regime(
    ...
    pair: str | None = None,  # NEW: optional pair-specific vol for regime scoring
    ...
    pair : str | None
        Optional currency pair (e.g. "EURUSD") for pair-specific vol scoring.
    ...
    # FIX SAT-08: pair-specific vol score when pair is provided
    if pair:
        pair_vol_key = f"{pair}_1M"
        pair_vol = vol_surface.get(pair_vol_key, 0.0)
        if pair_vol > 0:
            try:
                from app.engine_v1.pair_registry import get_pair_meta
                meta = get_pair_meta(pair)
                if meta.iso_region == "G10":
                    # G10: crisis at 15%+, stressed at 10%+
                    vol_score = max(0.0, (pair_vol - 5.0) / 15.0 * 100.0)
                else:
                    # EM: crisis at 30%+, stressed at 20%+
                    vol_score = max(0.0, (pair_vol - 10.0) / 25.0 * 100.0)
            except (ValueError, ImportError):
                pass  # Keep VIX-based score as fallback

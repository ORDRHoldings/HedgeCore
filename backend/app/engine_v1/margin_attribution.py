...
def compute_margin_attribution(
    ...
    pair: str | None = None,  # NEW: pair-specific stress multiplier
) -> MarginBreakdown:
    ...
    pair : str | None
        Optional currency pair for EM stress multiplier adjustment.

    Returns
    -------
    MarginBreakdown
    """
    # SAT-10: EM pairs get 25% higher stress multiplier
    if pair:
        try:
            from app.engine_v1.pair_registry import get_pair_meta
            meta = get_pair_meta(pair)
            if meta.iso_region in ("EM_LATAM", "EM_ASIA", "EM_CEEMEA"):
                scenario_stress_multiplier = scenario_stress_multiplier * 1.25

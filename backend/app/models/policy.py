...
class PolicyInstance(Base):
    ...
    __table_args__ = (
        ...
        # DB-POLICY-1: A PostgreSQL UNIQUE partial index WHERE is_active=TRUE is added via
        # Alembic migration c9f3a2b1d4e5 (uix_policy_instances_one_active_per_scope).
        # This enforces the one-active-per-scope invariant at DB level, preventing race
        # conditions. The service layer catches IntegrityError and raises 409 Conflict.

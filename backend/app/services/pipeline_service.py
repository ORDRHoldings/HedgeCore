...
def sandbox_calculate(user_id: str, request: SandboxCalculateRequest) -> dict:
    ...
    if hedge_plan:
        ...
        portfolio_equity = total_exposure * 0.1
...
async def create_proposal(session: AsyncSession, user_id: str, run_id: str) -> Proposal:
    ...
    residual_vector = [b.residual_mxn for b in response.hedge_plan.buckets]
    ...
    freeze_artifact_dict = {
        ...
        "exposure_digest": sha256_of_dict({
            "total_exposure": response.hedge_plan.summary.total_commercial_exposure_mxn,

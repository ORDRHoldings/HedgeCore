...
_COUNT_STMTS = {t: text("SELECT COUNT(*) FROM %s" % t) for t in _MONITORED_TABLES}  # noqa: UP031 — SEC-05 prohibits fstring inside text()
_MAX_CREATED_STMTS = {t: text("SELECT MAX(created_at) FROM %s" % t) for t in _MONITORED_TABLES}  # noqa: UP031
@router.get("/tables")

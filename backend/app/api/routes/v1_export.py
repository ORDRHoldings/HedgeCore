...
async def _write_export_audit(
    ...
    """Write a WORM audit event for a report export action (inline, awaited)."""
    try:
        prev_q = (
        ...
        try:
            roles = await rbac_service.get_user_roles(session, current_user.id)
            actor_role = (
                sorted(roles, key=lambda r: getattr(r, "hierarchy_level", 99))[0].name
                if roles else None
            )
        except Exception:
            actor_role = None
        ...
        event = build_audit_event(
            event_type      = "SYSTEM",
            description     = f"Report exported: run={run_id[:8]} format={export_format}",
            payload         = {
...
async def export_pdf(
    ...
    await _write_export_audit(session, current_user, run_id, "PDF")
...
async def export_excel(
    ...
    await _write_export_audit(session, current_user, run_id, "EXCEL")
...
async def export_zip(
    ...
    await _write_export_audit(session, current_user, run_id, "ZIP")

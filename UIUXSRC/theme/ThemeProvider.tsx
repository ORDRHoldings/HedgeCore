 ...
 *
 * Portable version: syncToServer is a no-op by default.
 * Pass onSyncToServer prop to ThemeProvider to enable server persistence.
 */
...
interface ThemeContextValue {
  ...
  /** Persist current appearance to server (no-op unless onSyncToServer is provided). */
...
interface ThemeProviderProps {
  children: ReactNode;
  /**
   * Optional callback for persisting appearance to a backend.
   * Called with the current appearance settings and auth token.
   * If not provided, syncToServer is a no-op.
   */
  onSyncToServer?: (appearance: AppearanceSettings, token: string) => Promise<void>;
}

// -- Provider --
export function ThemeProvider({ children, onSyncToServer }: ThemeProviderProps) {
  ...
  const syncToServer = useCallback(async (token: string) => {
    if (!onSyncToServer) return; // no-op by default
    try {
      await onSyncToServer(appearance, token);
    } catch { /* non-critical -- localStorage is primary */ }
  }, [appearance, onSyncToServer]);

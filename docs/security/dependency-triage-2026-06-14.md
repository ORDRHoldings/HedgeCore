...
**Outcome:** frontend ~70 alerts → **1 residual** (`xlsx`, no npm fix, not
exploitable here); backend → **2 deferred** (both with documented justification).
...
|---------|-----|---------------|-------------|
| `xlsx` | high | No patched version on the npm registry — SheetJS ships fixes only via its own CDN. | **Not exploitable here.** Both advisories (prototype pollution, ReDoS) require *parsing* attacker-controlled spreadsheets; this codebase uses `xlsx` **export-only** (`aoa_to_sheet`/`json_to_sheet`/`writeFile` in `frontend/src/utils/clientExport.ts` and `auditLabExport.ts`) — there is no `XLSX.read`/`readFile` of untrusted input. Formal clearance = migrate to the SheetJS CDN tarball (`npm i https://cdn.sheetjs.com/xlsx-<ver>/xlsx-<ver>.tgz`), a supply-chain change deferred for its own review (a URL dependency weakens `npm ci` reproducibility and cannot be CI-verified while billing is blocked). |
...
- Schedule the `xlsx` → SheetJS-CDN migration and the FastAPI/Starlette major upgrade
  as their own reviewed changes.
- After this lands, redeploy the frontend so the `next` 15.5.18 security patches reach
  production (`vercel deploy --prod` from repo root → project `hedgecore`).

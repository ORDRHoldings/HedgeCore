...
**FastAPI/Starlette upgrade attempt — BLOCKED, not shipped (draft PR #81).** Tried to clear the `starlette` 1.0 alert via a coordinated bump (fastapi 0.121→0.137, starlette 0.49→1.3, pydantic 2.12→2.13). Installs clean; full SQLite suite = **2 failed / 5387 passed / 161 skipped**. **Blocker:** FastAPI 0.137's `include_router` now inserts an opaque `_IncludedRouter` into `app.routes` instead of flattening — only **5** of the hundreds of routes are introspectable, so the RLS startup guards (`assert_routes_have_canonical_auth` / `assert_api_key_routes_safe`) **pass vacuously** (silent RLS-defense regression, the 2026-05-13 outage class) and `custom_openapi` would drop the business API. Caught by `test_router_registration_smoke` + `test_dashboard_routes_depend_on_get_current_user`. **Decision: do NOT ship** to the no-rollback prod backend on SQLite-only validation; deferred to a CI-validated PR once billing is restored. **RESOLVED same day via version choice:** the `_IncludedRouter` change is FastAPI **0.137.0-only** (released 2026-06-14, PR #15745); FastAPI **0.126–0.136** support starlette 1.x *without* it. Re-targeted PR #81 to **`fastapi==0.136.3`** (single-pin delta) — a clean drop-in: RLS guards enumerate all **455** routes (was 5), full SQLite suite **5514 / 160 / 0** (exact baseline), **zero code changes** needed. PR #81 marked **ready for review** (was draft), retitled. **MERGED to master 2026-06-15** (`ccb032c`) on explicit user instruction (`gh pr merge --admin`, CI billing-blocked) — production now on **fastapi 0.136.3 / starlette 1.3.1 / pydantic 2.13.4**. Backend auto-deploys to Render (deploys are health-gated, so a hard build/health failure keeps the old version live; pre-deploy prod was 200/`db:ok`). **Residual risk accepted, eyes-open:** SQLite-validated only — the PG-specific RLS runtime was not pre-validated (local PG present on `:5432`/`:5433` but creds unknown, not touched autonomously). The danger class is a subtle health-OK-but-broken-behavior regression (2026-05-13). **Rollback if prod misbehaves:** Render dashboard → redeploy previous, or `git revert ccb032c && push`. Full analysis: `docs/security/fastapi-starlette-upgrade-blocker-2026-06-14.md`. **Net Dependabot: 2 open** — `starlette` is now **fixed** by this merge; `ecdsa` (won't-fix upstream) + `pytest` 9 (deferred major) remain.
...
**Repo state**: merged to `master` via **PR #78** (merge `47277ae`; commits `5e7e04d` content + `1eeaa51` state, plus the carried session-40 rollups). CI hard-gates showed the documented billing-block signature — every job `steps:[]`, 2–9s wall-clock, verified via `gh run view 27483424682 --json jobs` (Backend "failed" in 2s, which cannot run 5,514 tests). Merged with `--admin` on local validation per CLAUDE.md §9.5 (same as PR #76/#77). **Production deploy DONE** — Vercel CLI was already authenticated as `synexiun` (the project owner), so deployed directly: `vercel link --yes --project hedgecore --scope synexiuns-projects` then `vercel deploy --prod --yes` from repo root (the documented known-good path; project `rootDirectory: frontend` builds server-side). Deployment `dpl_CWwybAbRYhNxAvArZk8VaPH5WDiU` READY, **aliased to `ordr-treasury.vercel.app`**. Live-verified: HTTP 200 + new markers (`Modules`, `Capabilities`, `63 permissions`, `Eight coordinated desks`, `Sage Intacct`) in served HTML; browser nav shows the Modules link + "60 Engine modules" stat. Also fixed this turn: `origin` remote URL repointed `Synexiun → ORDRHoldings/HedgeCore` (cleared the redirect warning).

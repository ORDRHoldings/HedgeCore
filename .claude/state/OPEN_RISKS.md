...
- **2026-06-07 audit re-confirm**: `LAUNCH_READINESS_RECONCILIATION_2026-06-07.md` blocker #10 re-confirmed this open (no tenant has live ERP creds).
- **Status**: Open
- **Opened**: 2026-04-14
...
- **2026-06-07 audit re-confirm**: `LAUNCH_READINESS_RECONCILIATION_2026-06-07.md` blocker #6 (Sentry 5xx alert + Render auto-rollback) re-confirmed this open.
...
- **2026-05-25 follow-up shipped (commit `4dda016`)** — stub migration `gg1a2b3c4d5e7_stub_companies_for_alembic_isolation` inserted between `g1a2b3c4d5e6` and `h1a2b3c4d5e6`. Pre-creates 5 ORM-only tables (`companies`, `legal_entities`, `permissions`, `roles`, `role_permissions`) via `CREATE TABLE IF NOT EXISTS`. Companion fixes: `h1.down_revision` rewired to gg1; `0013_add_sso_billing_to_companies` + `0017_legal_entities` made idempotent; `t1a2b3c4d5e6_add_ir_debt_permissions` UUID→SERIAL bug fixed; `migrations/env.py` widens `alembic_version.version_num` to VARCHAR(255). Chain reach in alembic-isolation: "crashes at h1a2b3c4d5e6" → "crashes at 0028_tca_permissions" — **~35 migrations of additional headroom**. STOPPED at this inflection: 0028/0030/0032 are individually-broken migrations (wrong `name` column, UUID/SERIAL mismatch) — content fixes do not amortize like stubs do.
- **Mitigation**: Already advisory (`continue-on-error: true`). RISK-CI-PG-01 milestone covers the larger fixture/schema audit.
- **Followups**: (a) ✅ DONE 2026-05-25 — workflow now mirrors production sequence (alembic non-fatal → `_ensure_tables` → `stamp head`). (b) Complete the migration guard sweep for remaining ORM-only tables when prioritized — now lower urgency since (a) is the primary fix. (c) Promote `backend-postgres` to hard gate (`continue-on-error: false`) — gated on the 83-fail count reaching zero (see verification below), not on N consecutive green runs.
- **Verified locally 2026-05-25** against fresh `postgres:16`: all three bootstrap steps complete; `pytest -m requires_postgres --maxfail=999` produces **66 passed / 83 failed / 5 errors / 5520 deselected** in 82s. The 83 failures are pre-existing fixture/auth/tenant-isolation issues that were hidden by the prior alembic crash. CI billing restoration will produce this same shape on the first real run. Working down the 83-fail list to zero is the path to hard-gate promotion.
- **2026-05-26 closure verification**: re-tested alembic-in-isolation on fresh probe2 PG (`DROP SCHEMA public CASCADE; CREATE SCHEMA public; alembic upgrade head`). **Chain now reaches `0036_force_rls_tenant_context (head)` cleanly with no crashes** — the later10 drain (env.py `transaction_per_migration` + 0028/0030/0032 permission content fixes) closed the chain-isolation gap that defined this risk. Full production-mirror bootstrap on the same fresh state: `alembic upgrade head` (clean) → `_ensure_tables` (idempotent) → marker suite → **154 passed / 5520 deselected / 0 failed in 112s**.
- **2026-06-07 note**: `LAUNCH_READINESS_RECONCILIATION_2026-06-07.md` blocker #3 (`_ensure_tables`→Alembic baseline) describes this same `_ensure_tables`-runs-first bootstrap pattern. It is the **accepted production bootstrap** documented here, not a defect — do NOT open a new risk for it. Kept explicitly DISTINCT from audit #4 (env.py model auto-discovery, ADR-0021, already FIXED).
- **Status**: CLOSED. The architectural root cause stated above ("chain crashes the moment it tries to ALTER an ORM-only table") is no longer true — every ORM-only table needed before chain-tail completion is either created by the `gg1a2b3c4d5e7` stub (companies/legal_entities/permissions/roles/role_permissions) or guarded with `pg_class` existence checks. The `set +e` workflow remains as defense-in-depth — production tolerates partial chain failure via the same swallow pattern, and the workflow guards against future migration content bugs without re-opening the risk.
- **Opened**: 2026-05-23  /  **Closed**: 2026-05-26
...
- **Followups (separate work)**:
  1. Audit which of the 237 tests are genuinely E2E vs which should be component tests.
     - **2026-05-27 (later16)**: structural triage complete — all 51 specs read and bucketed. Distribution: **E2E_GENUINE 5 files (~68 tests)** (audit-lab-workflow, audit_lab_e2e, hedge_desk_e2e_full, hedge_execution_flow, position_lifecycle) | **SMOKE_PAGE_PAINT 31 files (~143 tests)** dominated by treasury-suite/* (12 single-route specs + the umbrella treasury-suite.spec.ts at 19 routes), dashboard, market, governance-suite, settings, appearance-settings | **COMPONENT_LEVEL ~5 files** (quickstart_window, quickstart_accessibility, theme-system, governance API-interception) candidates for jest+RTL migration | **BRITTLE_ORPHAN — claimed 10 files, but bucket is UNRELIABLE: per-file verification required before any deletion**. Subagent flagged `happy_path.spec.ts` and `invalid_input.spec.ts` as orphans based on the comment string "Replaces legacy test that used non-existent /policy-desk" — but reading the files shows that comment means **this file is the replacement**, not that this file is the orphan. Both target canonical `/position-desk` routes confirmed in later13. Lesson: the heuristic "comment mentions a deleted route" is a false positive for the cohort of test files that document the route migration in their own header. The drive-by-deletions playbook applies recursively to test-file triage.
  ...
  4. **2026-05-27 derived from triage**: 31 SMOKE_PAGE_PAINT specs are structurally identical to nav-smoke (single `page.goto` + body-visible assertion, no mutations, no cross-route flows). Candidates for additive promotion into the `smoke` project by widening `testMatch` to cover `treasury-suite/*`, `dashboard/*`, `market/*`, `governance/*`. Additive — does not remove anything from the full chromium suite, only expands the smoke project's coverage. Defer until smoke job has demonstrated N consecutive green runs in real CI (currently blocked on billing).
  5. **2026-05-29 IN PROGRESS — per-file BRITTLE_ORPHAN verification (session 38, paused on user `save`)**: Verified 11 of ~14 explicitly-flagged orphan candidates as canonical-route specs targeting valid pages — NO deletions warranted. Confirmed canonical: `policy_desk_confirmation`, `decision-desk`, `phase_complete_reports`, `rejection_path`, `position_persistence`, `export_report`, `audit-lab`, `accounting`, `auth`, `reports-market-research`, `support_tickets`. Open verification: three navigateAuth targets inside `treasury-suite.spec.ts` (lines 16/22/40) — `/hedge-monitor`, `/trade-history`, `/hedge-templates` — do not appear in the `frontend/src/app/**/page.tsx` grep (timed out; needs retry with narrower glob). **Resume action**: confirm whether these three are dead route refs (then surgical line-level edit inside `treasury-suite.spec.ts`, NOT spec deletion) or whether the page-list grep was incomplete. Then write up the verification table replacing the "bucket unreliable, deferred" line above with a verified ✓/✗ matrix.
- **2026-06-07 audit re-confirm**: `LAUNCH_READINESS_RECONCILIATION_2026-06-07.md` blocker #9 (E2E expansion) re-confirmed this open.

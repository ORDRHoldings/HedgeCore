...
- **Followups**: Consider hoisting `_resolve_user` into `core/dependencies.py` or replacing it with `Depends(get_current_user)` to eliminate the parallel auth helper entirely. Dashboard routes are currently allowlisted in the RLS-02 guard with a comment noting the deferred refactor.
- **Mitigation B (structural defense)**: 2026-05-24 — shipped `assert_routes_have_canonical_auth(app)` companion to the RLS-01 guard (commit `4607acc`). Walks every APIRoute and requires either `get_current_user` or `get_api_key_principal` in the dependant tree, or explicit listing in `NO_AUTH_ROUTE_ALLOWLIST` (42 entries, categorized: root/docs, health, auth-issuance, webhooks, public market data, seed, engine, dashboard). 9 new tests in `backend/tests/test_canonical_auth_startup_guard.py` including a regression boot of the production app against its own guard. This is the structural defense that would have caught the original `_resolve_user` regression before merge.
- **Status**: CLOSED (mitigated 2026-05-24, hardened 2026-05-24). Tests: 5504+3+9 backend passing.
